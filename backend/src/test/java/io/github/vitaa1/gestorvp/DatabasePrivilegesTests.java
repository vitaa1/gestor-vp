...
@SpringBootTest(properties = {
		...
		"spring.flyway.user=gestor_vp_migration",
...
class DatabasePrivilegesTests {
	...
	void runtimeCanUseApplicationObjectsButCannotChangeSchemaOrFlywayHistory() throws SQLException {
		...
		try (var connection = DriverManager.getConnection(
				postgresContainer.getJdbcUrl(), "gestor_vp_runtime", "runtime-test-password");
	...
	private boolean hasTablePrivilege(String table, String privilege) {
		return Boolean.TRUE.equals(jdbcTemplate.queryForObject(
				"select has_table_privilege('gestor_vp_runtime', ?, ?)",
				Boolean.class,
				"public." + table,
				privilege));
	}
	...
	private boolean hasSequencePrivilege(String sequence, String privilege) {
		return Boolean.TRUE.equals(jdbcTemplate.queryForObject(
				"select has_sequence_privilege('gestor_vp_runtime', ?, ?)",
				Boolean.class,
				"public." + sequence,
				privilege));
	}
	...
	private boolean hasSchemaPrivilege(String privilege) {
		return Boolean.TRUE.equals(jdbcTemplate.queryForObject(
				"select has_schema_privilege('gestor_vp_runtime', 'public', ?)",
				Boolean.class,
				privilege));
	}
}
